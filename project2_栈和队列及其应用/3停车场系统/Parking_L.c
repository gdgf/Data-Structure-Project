...
void Arrive(SqStackCar *stop,LinkQueueCar *lq,CarNode x)
{
	...
	if (f==0){                //栈满
		...
		printstop(1,lq->front->num,0,23);     //去停车
...
{
	Time *t;
	time_t timer;        //time_t is 这是一个适合存储日历时间类型。
    struct tm *tblock;   //struct tm 这是一个用来保存时间和日期的结构。
	t=(Time*)malloc(sizeof(Time));

    timer=time(NULL);     //获取字1970年以后到现在经历的时间（多少秒）
    tblock=localtime(&timer);   //变成本地时间
...
{
	COORD coord;     //COORD，头文件windows里面的移动光标的结构体
	coord.X=x;
	coord.Y=y+3;
	SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE),coord);
	/*SetConsoleCursorPosition:通过修改pos.X和pos.Y的值就可以实现光标的位置控制。
      GetStdHandle:它用于从一个特定的标准设备（标准输入、标准输出或标准错误）
      中取得一个句柄（用来标识不同设备的数值）。可以嵌套使用。
